...
body_t *new_orbiting(Vector2 perihelion, Vector2 aphelion, float mass,
                     float radius, Color color, const char *name) {
    ...
    // distances from Sun
    float r_peri =
        sqrtf(perihelion.x * perihelion.x + perihelion.y * perihelion.y);
    ...
    // compute tangential direction perpendicular to radius vector from Sun
    // (0,0)
